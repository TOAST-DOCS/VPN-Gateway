...
* VPN 연결에 사용되는 대역은 /29 이하여야 하고, 내부적으로 각 대역의 마지막 주소 4개를 사용하므로 해당 주소는 할당해서 사용하면 안됩니다. 예를 들어 /29 를 사용하는 경우 x.x.x.4, x.x.x.5, x.x.x.6, x.x.x.7 4개 주소는 사용하면 안됩니다. /30부터는 내부 시스템에서 할당해야 하는 주소자체가 모자라므로 사용이 불가능 합니다.
...
* 한국(평촌) 리전과 한국(판교) 리전 간에도 VPN 연결을 생성할 수 있습니다(피어 게이트웨이 장비를 서로 Fortinet 선택).
